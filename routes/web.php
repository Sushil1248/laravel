...
use App\Http\Controllers\Admin\{DashboardController,UserController,MediaController,CmsManagementController,SubscriptionController,PaymentController,CompanyController, RoleController};
use App\Http\Controllers\{HomeController,TestingController, TestFCMController, VehicleController};
...
Route::group([ ], function () use ($router) {
    ...
    $router->middleware(['auth', 'role:Administrator'])->group(function () use ($router) {
    ...
    $router->middleware(['auth', 'role:Company|Administrator'])->group(function () use ($router) {
        $router->prefix('user')->name('user.')->group(function() use($router) {
            $router->get('devices/{user_id?}', [UserController::class,"getDevices"])->name('devices')->middleware('permission:user-list');
            $router->match(['get','post'], 'add/device', [UserController::class,"addDevice"])->name('adddevice')->middleware('permission:user-list');
            $router->match(['get','post'], 'add', [UserController::class,"add"])->name('add')->middleware('permission:user-add');
            ...
            $router->post('notify/device/', [UserController::class,"sendPushNotification"])->name('device.push-notification')->middleware('permission:user-edit');
            $router->get('restore/{id}', [UserController::class,"del_restore"])->name('restore')->middleware('permission:user-delete');
            $router->get('device/restore/{id}', [UserController::class,"device_restore"])->name('restoreDevice')->middleware('permission:user-delete');
            $router->get('details/{id}', [UserController::class,"view_detail"])->name('details')->middleware('permission:user-view');
            $router->get('export', [UserController::class,"export"])->name('export')->middleware('permission:user-list');
        ...
        $router->match(['post'], 'edit/{id?}', [CompanyController::class,"edit"])->name('c.edit')->middleware('permission:user-edit');
    });


    // Company Routes
    $router->middleware(['auth', 'role:Company'])->prefix('c')->group(function () use ($router) {
        ...
        $router->get('list/{deleted?}', [CompanyController::class,"getUserList"])->name('list')->middleware('permission:user-list');


        /* ***************************** vehicles route *************************************** */
        $router->prefix('vehicle')->name('vehicle.')->group(function() use($router) {
            $router->get('list/{deleted?}', [VehicleController::class,"getList"])->name('list')->middleware('permission:user-list');
            $router->match(['get','post'], 'add', [VehicleController::class,"add"])->name('add')->middleware('permission:user-add');
            $router->get('changeStatus', [VehicleController::class,"changeStatus"])->middleware('permission:user-status');
            $router->match(['get','post'], 'edit/{id?}', [VehicleController::class,"edit"])->name('edit')->middleware('permission:user-edit');
            $router->match(['get','post'], 'update-password/{id}', [VehicleController::class,"updatePassword"])->name('update-password')->middleware('permission:user-edit');
            $router->get('delete/{id}', [VehicleController::class,"del_record"])->name('delete')->middleware('permission:user-delete');
            $router->get('restore/{id}', [VehicleController::class,"del_restore"])->name('restore')->middleware('permission:user-delete');
            $router->get('details/{id}', [VehicleController::class,"view_detail"])->name('details')->middleware('permission:user-view');
            $router->get('export', [VehicleController::class,"export"])->name('export')->middleware('permission:user-list');
        });
