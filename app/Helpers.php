...
use App\Models\User;use App\Notifications\CommonNotification;use Carbon\Carbon;use Illuminate\Support\Facades\Crypt;
...
if (!function_exists('company_user_array')) {
    function company_user_array()
    {
        $companyId = Auth::user()->id;
        $companyUsers = User::whereHas('companyUsers', function ($query) use ($companyId) {
...
function trim_role_name($name)
{
    $str = $name;
    $parts = explode('_', $str);
    $value_after_underscore = $parts[1];
...
function makeCurlRequest($url, $requestType, $params)
{
    $curl = curl_init();
